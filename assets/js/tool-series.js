document.addEventListener('DOMContentLoaded', function() {
  ...
  // Extract URL fragment without the '#'
  var defaultFilter = window.location.hash.replace('#', '');
  
  // Set the default filter button as 'is-checked'
  var defaultButton = defaultFilter 
    ? document.querySelector(`.category-filters [data-filter="${defaultFilter}"]`) 
    : document.querySelector('.category-filters [data-filter="all"]');

  if (defaultButton) {
    defaultButton.classList.add('is-checked');
    var filterValue = defaultButton.getAttribute('data-filter');
